...
```
The first field in this register tells the graphics hardware that the framebuffer is using colors stored in the pallette from index number 17 to 255. This is essentially an offset added onto the color indecies stored in the framebuffer. We need to leave the first 16 color palette entries for the tiles used for the text console. Down the road if you don't want to use those tiles, or replace them you can use the full color palette.
...
Now you can try out the palette that you have created. Just fill the framebuffer memory with values from 0 to 238. Remember that we only have 239 indecies avialable as the first 16 are used by the text console tiles.
...
We can use another handy hardware feature. The random number generator. If you read a value from `MISC_REG(MISC_NRG_REG)` you will be provided with a word full of random bits.

Use those random bits to fill the last line of the framebuffer (`y = FB_HEIGHT -1`) with 0 or 238. You can use the `FB_PIX` to make access to the frame buffer pixel entries bit easier.
...
If you read throught the diff you would see that there is a new definition in the `gloss` hardware register header. It is called `GFX_LAYEREN_FLT_GRAY` and can be set in the `GFX_LAYEREN_REG` register.
